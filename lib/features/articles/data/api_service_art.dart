...
class ApiServiceArt {
  ...
  Future<List<dynamic>> getAllPosts() async {
    try {
      // بناءً على هيكل مساراتك، هذا هو المسار المتوقع
      final response = await dio.get('/api/user/showallPost');
      return response.data as List<dynamic>;
    } on DioException catch (e) {
      if (e.response?.statusCode == 404) return [];
      throw Exception(e.response?.data['message'] ?? 'خطأ في جلب المقالات');
    }
  }
}